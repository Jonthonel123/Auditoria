        ...
        if ($this->session->userdata('logged_in') === TRUE) {
            if ($this->input->post()) {
                ...
                if ($this->Persona_model->update($id_registro, $responsable)) {
                    ...
                    $this->alert("La información fue actualizada correctamente", site_url($this->config->item('path_backend') . '/responsables'));
                }
            }
            ...
            $arrayMigaPan = array(
                array("nombre" => "Responsable", "url" => site_url($this->config->item('path_backend') . '/responsables')), array("nombre" => "Editar", 'active' => true)
            );
